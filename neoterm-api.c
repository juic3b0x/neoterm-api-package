...
#define NEOTERM_API_PACKAGE_VERSION "0.58.0"

#ifndef PREFIX
# define PREFIX "/data/data/io.neoterm/files/usr"
#endif

#define LISTEN_SOCKET_ADDRESS "io.neoterm.api://listen"
                                 ...
                                 char* output_address_string)
{
    ...
    // On Android `>= 14`, if neoterm-api app process was started previously
    ...
    // - https://github.com/juic3b0x/neoterm-api/issues/638#issuecomment-1813233924
    int listenfd = -1;
    #ifdef __ANDROID__
                                 ...
                                 char* output_address_string)
{
    ...
    child_argv[3] = "0";
    child_argv[4] = "-n";
    child_argv[5] = "io.neoterm.api/.NeoTermApiReceiver";
    child_argv[6] = "--es";
...
_Noreturn void exec_callback(int fd)
{
    ...
    /* NEOTERM_EXPORT_FD and NEOTERM_USB_FD are (currently) specific for
       neoterm-usb, so there's some room for improvement here (this
       function should be generic) */
    char errmsg[256];
    char *export_to_env = getenv("NEOTERM_EXPORT_FD");
    if (export_to_env && strncmp(export_to_env, "true", 4) == 0) {
        if (setenv("NEOTERM_USB_FD", fds, true) == -1)
            perror("setenv");
        execl(PREFIX "/libexec/neoterm-callback", "neoterm-callback", NULL);
        sprintf(errmsg, "execl(\"" PREFIX "/libexec/neoterm-callback\")");
    } else {
        execl(PREFIX "/libexec/neoterm-callback", "neoterm-callback", fds, NULL);
        sprintf(errmsg, "execl(\"" PREFIX "/libexec/neoterm-callback\", %s)", fds);
    }
    perror(errmsg);
    exit(1);
}
